...
TMURF (Tell Me yoU aRe Finish) 是一个 **Tampermonkey 用户脚本**，用于监听 Deepseek 聊天页面的 API 请求。当 AI 完成请求时，自动发送浏览器通知。
...
| 文件 | 用途 |
|------|------|
| `tampermonkey.js` | 核心用户脚本（唯一源代码文件） |
...
- `@grant GM_notification` 和 `@grant GM_addStyle`：声明 Tampermonkey API 权限

### 代码模式
- **IIFE 封装**：使用 `(function() { "use strict"; })()` 避免全局污染
- **XHR 拦截**：保存原始 `XMLHttpRequest.prototype.open/send`，在 `send` 中监听 `readyState === 4`
- **去重机制**：`NOTIFICATION_COOLDOWN = 5000`（5 秒冷却时间）
- **页面可见性**：仅在 `document.hidden === true` 时发送通知

### 命名约定
- 私有变量/方法使用 `_tmurf` 前缀（如 `_tmurfMethod`, `_tmurfURL`, `_tmurfNotified`）
- 常量使用 `UPPER_SNAKE_CASE`（如 `NOTIFICATION_COOLDOWN`）
- 函数使用 `camelCase`（如 `sendNotification`, `isChatAPIRequest`）

## 重要注意事项

### ⚠️ 开发环境
- **无构建系统**：这是纯 JavaScript 文件，直接编辑 `tampermonkey.js` 即可
- **无测试框架**：手动在浏览器中测试（安装 Tampermonkey 扩展后加载脚本）
- **无包管理**：不依赖 npm 或其他包管理器
...
4. **URL 类型**：`isChatAPIRequest` 同时处理字符串和 `URL` 对象

### ⚠️ 修改建议
- 添加新功能时，保持 IIFE 封装和严格模式
- 拦截逻辑修改后，确保调用原始方法（`originalXHROpen.apply`, `originalXHRSend.apply`）
- 通知逻辑修改时，保留去重和页面可见性检查

## 测试方法

1. 在浏览器中安装 [Tampermonkey](https://www.tampermonkey.net/)
2. 创建新脚本，粘贴 `tampermonkey.js` 内容
